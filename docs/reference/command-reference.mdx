...
Agents should follow the repository-installed `visor-discovery` skill: read compact agent memory before visual evidence, use `route` for multi-step navigation, and use `discover` only to name or repair unknown live states. See [Agent memory and deterministic routes](/features/app-map).
