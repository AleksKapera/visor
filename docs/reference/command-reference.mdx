...
Use `--annotate-current <file|->` to observe the current screen and merge semantic screen/action descriptions into that same observed variant. Pass a UTF-8 JSON file or `-` for one JSON value from standard input. Annotation input is validated before device selection or daemon work, and the update is applied before an optional crawl changes app state.
