...
| Field | Type | Meaning |
| --- | --- | --- |
| `screen_applied` | boolean | Whether screen semantics were applied |
...
The same response includes `data.screen.variant_id` and `data.screen.screen_id`. Annotation is applied before `data.crawl` when both options are used.
