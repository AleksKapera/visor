...
For agent-driven work, treat these commands as execution primitives rather than a discovery strategy. Read compact [agent memory](/features/app-map) first, reuse a verified direct action for one known step, and use one deterministic route for a known multi-step goal. Request screenshot or source only when the map cannot resolve an ambiguity.
