...
- agent-supplied screen labels, purposes, action labels, intents, safety, descriptions, and notes
...
Use `visor discover --annotate-current <file|->` to attach semantic screen and action descriptions to the screen observed by that same command. Stable screen labels and purposes are shared with sufficiently similar variants on the same logical screen; variant descriptions and notes remain specific. Action identity uses the Visor command plus normalized arguments, so repeated input updates an existing action instead of creating a duplicate. When a source action matches, Visor retains its selector, geometry, and scope while merging the semantic fields. Unmatched actions are stored as ordinary map facts without provenance or match markers.

Visor validates annotation JSON before selecting a device or contacting the daemon. Annotation text follows the map's privacy redaction, and typed `act` values are removed before persistence. The response acknowledges the screen update and inserted, updated, or merged action counts.
