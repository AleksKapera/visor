...
For scenario execution and direct action commands in real runtime mode, start the Visor daemon first:

```bash
visor start
```

`visor start` ensures Appium is reachable and starts a local daemon that keeps WebDriver sessions warm for subsequent commands. Real runtime commands fail early if the daemon is not running. Mock commands do not require the daemon.
